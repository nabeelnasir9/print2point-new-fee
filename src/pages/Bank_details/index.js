...
const Bank_details = () => {
  ...
  const checkStripeAccount = async () => {
    ...
    if (connect.data.status) {
      setStatus(connect.data.status);
    }
          ...
          <div class="container">
            ...
            {status && (
              ...
              <div style={{ marginTop: "10px" }}>
                ...
                <p>
                  Requirements: {status.requirements?.errors?.length
                    ? status.requirements.errors.map((error, index) => (
                      <div key={index}>
                ...
                <p>
                  Verification: {status.requirements.pend || 'Pending'}
                </p>

                {status.requirements?.errors?.length || status.requirements?.eventually_due?.length ? <button class="reconnect" onClick={connectStripe}>Reconnect to Stripe</button> : null}
              </div>
            )}
          </div>
      }
